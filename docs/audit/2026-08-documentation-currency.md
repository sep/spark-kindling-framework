...
**Method:** one background research pass reading every proposal doc, `config_reference.md`, `databricks_execution_contract.md`, `cli_reference.md`, and the referenced provider/CLI source, verifying each claim against `grep`/file-existence checks rather than trusting prose.
...
`docs/proposals/config_driven_execution_options.md` self-reports Phases 1–3 shipped (#169, #173), and the code confirms it: `generation_executor.py:505,583,604` reads `kindling.execution.{parallel,max_workers,error_strategy,pipe_timeout,auto_cache,retry.*}` and per-pipe `kindling.execution.pipes.<pipeid>.retry.*`. This is a fully-shipped, actively-used config surface with zero entries in `config_reference.md`.
...
Verdict per file (excludes `docs/proposals/obsolete/`), verified by grepping for the described feature/module rather than trusting the doc's own status header.

| Proposal | Verdict |
|---|---|
| `app_settings_overlay_model.md` | Mostly implemented — self-reported accurately |
| `comprehensive_tracing_instrumentation.md` | Fully implemented — self-reported accurately |
| `config_driven_execution_options.md` | Fully implemented (`generation_executor.py`) — but see "missing docs" above, its own config surface is undocumented |
| `dataproc_platform_evaluation.md` | Not started (evaluation-only; no Dataproc code exists) — consistent with its own status |
| `declarative_pipelines_engine.md` | Partially implemented — Phase 1 (SDP) shipped; parent doc correctly still "Draft" |
| `entity_provider_roadmap.md` | Fully implemented and accurately self-updated (ADX, Cosmos, Parquet all present with system tests) |
| `event_condition_episode_ontology.md` | Conceptual/ontology doc, not directly code-mapped |
| `event_hub_kafka_transport.md` | Partially implemented, **status understates it** — marked "Proposed," but `provider.transport` (auto/eventhubs/kafka) already exists and defaults to kafka on Databricks; only the cross-platform default-to-kafka policy remains open |
...
| `kindling_patterns_cli.md` | Not started — being retired to `obsolete/` via PR #204 (superseded by `app init --pattern` direction) |
| `load_workspace_packages_rename.md` | Fully implemented — self-reported accurately; one leftover (removing deprecated `load_local` alias) tracked by issue #202 |
| `memory_provider_seed_rows.md` | Fully implemented — already documented in `config_reference.md` |
...
| `package_config_architecture.md` | Partially implemented — Phases 1–2 (`ConfigPatternMatcher`, overlay) shipped and verified live; Phase 3 (`_enabled`/`_remove_tags` interpretation, introspection API) confirmed absent despite a code comment claiming otherwise (`config_patterns.py:46`) — tracked by issue #32 |
| `rest_api_entity_provider.md` | Not started — tracked by issue #200 |
| `sdp_engine_phase1_notes.md` | Fully implemented (Phase 1) — implementation record, self-reported accurately |
| `stage_processor_signal_inversion.md` | Not started — being retired to `obsolete/` via PR #204 (framework adopted aspect-on-persist-signals direction instead) |
| `surrogate_keys.md` | Not started (Phase 1 only) — tracked by issue #199 |
| `temporal_condition_sources.md` | 🔴 **Fully implemented but still marked "Proposed."** `condition_source` (`table`/`registry`) and `DataConditions.register(...)` exist exactly as proposed in `kindling_ext_temporal/registry.py:54,239-262`, `chain.py:365-366`, `translation.py:155,375` — tracked/closed by issue #222. **Status header still needs updating** as of this write-up. |
...
None of the "Stale documentation" or "Missing documentation" items above have been fixed yet — this write-up is the tracking record for that work, not a changelog of completed fixes. The three "Now tracked" proposal gaps already have GitHub issues filed (#245, #246, #247); the doc-staleness items do not yet have issues and are tracked only here.
