# Platform Job Deployment Internals

## Overview

This document describes the internal platform job primitives Kindling uses to
install and invoke the durable runner across Fabric, Databricks, and Synapse.
End users should run apps with `kindling app run` and manage runner
infrastructure with `kindling runner ensure/status/repair/delete`.

These APIs are framework internals and SDK/platform-adapter building blocks.
They are not a separate end-user CLI workflow and do not imply that generated
apps should include a `job.yaml`.
