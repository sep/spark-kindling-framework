...
if add_to_registry:
    ...
    def _initialize_cache(self):
        ...
        try:
            ...
            self.logger.debug(
                f"Cache initialized with {len(self._items_cache)} total items")
    ...
    def copy(self, source: str, destination: str, overwrite: bool = False) -> None:
        ...
        if mssparkutils:
            ...
            raise NotImplementedError(
                "File copy not available without mssparkutils")
    ...
    def delete(self, path: str, recurse=False) -> None:
        ...
        if mssparkutils:
            ...
            raise NotImplementedError(
                "File delete not available without mssparkutils")
    ...
    def list(self, path: str) -> List[str]:
        ...
        if mssparkutils:
            ...
            raise NotImplementedError(
                "Directory listing not available without mssparkutils")
    ...
    def create_or_update_notebook(self, notebook_name: str, notebook_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            notebook_resource = self._convert_to_synapse_notebook(
                notebook_name, notebook_data)
    ...
    def delete_notebook(self, notebook_name: str) -> None:
        """Delete a notebook"""
        try:
            operation = self.client.notebook.begin_delete_notebook(
                notebook_name)
            operation.result()  # Wait for completion
            self.logger.debug(
                f"Successfully deleted notebook: {notebook_name}")
        except Exception as e:
            self.logger.error(
    ...
    def _convert_to_notebook_resource(self, synapse_data: Dict[str, Any], include_content: bool = True) -> NotebookResource:
        ...
        if include_content and 'definition' in synapse_data:
            ...
            notebook_content = self._ensure_safe_notebook_content(
                notebook_content)
            ...
            notebook_content = self._extract_notebook_content_from_synapse(
                properties)
            notebook_content = self._ensure_safe_notebook_content(
                notebook_content)
            notebook_content['folder'] = folder_info
