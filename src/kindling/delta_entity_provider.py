...
class DeltaEntityProvider(EntityProvider):
    ...
    def _ensure_schema_applied(self, entity, table_ref: DeltaTableReference):
        ...
        try:
            ...
            if len(existing_fields) == 0:
                self.logger.warning(f"Table {table_ref.table_name} exists but has no schema, applying schema from entity definition")
                ...
                empty_df.write.format("delta") \
                    ...
                    .save(table_ref.table_path)
                
                self.logger.info(f"Schema applied to {table_ref.table_name}")
            else:
                self.logger.info(f"Table {table_ref.table_name} already has schema with {len(existing_fields)} columns")
    ...
    def _ensure_table_exists(self, entity, table_ref: DeltaTableReference):
        ...
        self.logger.info(f"Table status for {table_ref.table_name}: physical={physical_exists}, catalog={catalog_exists}")
    ...
    def _create_physical_table(self, entity, table_ref: DeltaTableReference):
        ...
        try:
            ...
            if current_db != "default":
                self.logger.debug(f"Switching from database '{current_db}' to 'default' for table creation")
            ...
            self.logger.error(f"Failed to create physical Delta table at {table_ref.table_path}: {e}")
            raise
    ...
    def _attempt_catalog_registration(self, table_ref: DeltaTableReference):
        ...
        try:
            ...
            if "null path" in error_msg.lower() or "hiveexception" in error_msg.lower():
                ...
                database = table_ref.table_name.split(".")[0] if "." in table_ref.table_name else "unknown"
                self.logger.error(
                ...
                # Some other error
                self.logger.error(f"Could not register table {table_ref.table_name} in catalog: {e}")
            
            # For FOR_NAME mode, this is a problem but don't fail - table still works via path
            if self.access_mode == DeltaAccessMode.FOR_NAME:
                self.logger.warning(f"FOR_NAME mode requested but catalog registration failed. Table will work via path access only.")
