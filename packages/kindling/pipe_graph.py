...
class PipeEdge:
    ...
    def __eq__(self, other):
        if not isinstance(other, PipeEdge):
            return False
        return (
            self.from_pipe == other.from_pipe
            and self.to_pipe == other.to_pipe
...
class PipeGraph:
    ...
    def get_root_nodes(self) -> List[str]:
        ...
        return [pipe_id for pipe_id in self.nodes.keys() if not self.get_dependencies(pipe_id)]

    def get_leaf_nodes(self) -> List[str]:
        """Get pipes with no dependents (sinks)."""
        return [pipe_id for pipe_id in self.nodes.keys() if not self.get_dependents(pipe_id)]
    ...
    def get_shared_inputs(self) -> Dict[str, List[str]]:
        ...
        return {
            entity: consumers
            for entity, consumers in self.entity_consumers.items()
            if len(consumers) > 1
        }
...
class GraphCycleError(Exception):
    ...
    def __init__(self, cycle: List[str]):
        # Normalize to an "open" cycle (no duplicated closing node)
        if cycle and cycle[0] == cycle[-1]:
            open_cycle = cycle[:-1] or cycle
        else:
            open_cycle = cycle

        self.cycle = open_cycle

        # For display, always close the loop once
        display_cycle = open_cycle + [open_cycle[0]] if open_cycle else []
        cycle_str = " → ".join(display_cycle)
        super().__init__(
            f"Cycle detected in pipe dependencies: {cycle_str}\n"
            f"Pipes in cycle: {', '.join(open_cycle)}"
        )
...
class PipeGraphBuilder:
        ...
        for pipe_id, node in graph.nodes.items():
            for input_entity in node.input_entities:
                ...
                if producer_id and producer_id in graph.nodes:
                    ...
                    graph.add_edge(edge)
                    self.logger.debug(f"Added edge: {producer_id} → {pipe_id} (via {input_entity})")
        ...
        def dfs(node_id: str) -> bool:
            ...
            for neighbor in graph.get_dependents(node_id):
                if neighbor not in visited:
                    ...
                    # Found cycle - extract it from path as an 'open' cycle
    ...
    def topological_sort(self, graph: PipeGraph) -> List[str]:
        ...
        in_degree = {
            pipe_id: len(graph.get_dependencies(pipe_id)) for pipe_id in graph.nodes.keys()
        }
    ...
    def get_generations(self, graph: PipeGraph) -> List[List[str]]:
        ...
        while len(processed) < len(graph.nodes):
            ...
            current_generation = [
                ...
                and all(dep in processed for dep in graph.get_dependencies(pipe_id))
            ]
            ...
            self.logger.debug(f"Generation {len(generations)}: {len(current_generation)} pipes")
    ...
    def get_graph_stats(self, graph: PipeGraph) -> Dict[str, any]:
        ...
        return {
            ...
            "total_cache_opportunities": sum(
                len(consumers) for consumers in shared_inputs.values()
            ),
        }
