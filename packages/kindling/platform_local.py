...
class LocalService(PlatformService):
    ...
    - Spark session runs in local mode (e.g., local[*])

    The local platform is designed for:
    - Development and testing without cloud dependencies
    - Running unit tests with local Spark
    - Local data processing and validation
    - CI/CD pipelines that don't require cloud services
    """

    def __init__(self, config, logger):
        self.config = config if isinstance(
            config, types.SimpleNamespace) else types.SimpleNamespace(**config)
        self.logger = logger

        # Local platform attributes
        ...
        default_workspace = Path(tempfile.gettempdir()) / \
            "kindling_local_workspace"
    ...
    def _initialize_cache(self):
        ...
        try:
            ...
            self.logger.debug(
                f"Local cache initialized with {len(self._items_cache)} items")
    ...
    def get_environment(self):
        ...
        return os.environ.get("PROJECT_ENV", os.environ.get("ENVIRONMENT", "local"))

    def get_token(self, audience: str = None) -> str:
        """Get access token (not needed for local - returns dummy token)"""
        self.logger.debug(
            f"Local platform does not use real tokens. Audience: {audience}")
    ...
    def copy(self, source: str, destination: str, overwrite: bool = False) -> None:
        ...
        source_path = Path(source) if Path(
            source).is_absolute() else self.local_workspace_path / source
        dest_path = Path(destination) if Path(
            destination).is_absolute() else self.local_workspace_path / destination
    ...
    def delete(self, path: str, recurse: bool = False) -> None:
        ...
        target_path = Path(path) if Path(
            path).is_absolute() else self.local_workspace_path / path
    ...
    def list(self, path: str) -> List[str]:
        ...
        dir_path = Path(path) if Path(path).is_absolute(
        ) else self.local_workspace_path / path
        ...
        if include_content:
            ...
            try:
                ...
                if file_path and Path(file_path).exists():
                    ...
                    notebook_data['properties']['cells'] = self._convert_python_to_cells(
                        content)
                else:
    ...
    def update_notebook(self, notebook_name: str, content: str) -> bool:
        ...
        try:
            ...
            self.logger.error(
                f"Failed to update notebook {notebook_name}: {e}")
            return False
    ...
    def delete_notebook(self, notebook_name: str) -> bool:
        ...
        try:
            ...
            self.logger.error(
                f"Failed to delete notebook {notebook_name}: {e}")
            return False
    ...
    def create_or_update_notebook(self, notebook_name: str, notebook_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(
                f"Failed to create/update notebook {notebook_name}: {e}")
            raise
    ...
    def is_interactive_session(self) -> bool:
        ...
        try:
            connection_file = ipython.config.get(
                'IPKernelApp', {}).get('connection_file')
...
# Register the local platform service
@PlatformServices.register(
    name="local",
    description="Local platform service for development and testing")
