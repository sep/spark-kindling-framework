...
# Conditional pytest import - only required for actual testing
try:
    import pytest
except ImportError:
...
class InjectorServiceWrapper:
    ...
    def get_combined_framework(
        environment_service=None, notebook_loader=None, config=None
    ) -> CombinedFramework:
        """
        ...
        if "framework" in globals():
            ...
            return CombinedFramework(fw.environment_service, fw.notebook_loader, fw.config)
...
class NotebookTester:
    ...
    def _get_logger(self):
        ...
        return type(
            "Logger", (), {"info": print, "error": print, "debug": print, "warning": print}
        )()

    def run_notebook_suites(
        self, test_suites: Dict, test_suite_configs: Optional[Dict] = None
    ) -> Dict:
        """
        ...
        for key in test_suites.keys():
            ...
            results[key] = self.run_notebook_tests(
                *test_suites[key], test_config=current_config, env=env
            )
        return results
            ...
            try:
                ...
                if pytest_cell_code:
                    test_suites[notebook.name] = self._execute_test_cell_with_imports(
                        pytest_cell_code, notebook.name
                    )
                    self.logger.info(
                        f"Found {len(test_suites[notebook.name])} tests in suite {notebook.name}"
                    )
    ...
    def _extract_pytest_cell(self, notebook) -> Optional[str]:
        ...
        for cell in notebook.properties.cells:
            if cell.cell_type == "code":
                ...
                if (
                    cell_code.strip().startswith("%run")
                    and "environment_bootstrap" not in cell_code
                    and "test_framework" not in cell_code
                ):
                    code = code + "\n" + cell_code
        ...
        try:
            ...
            for name, obj in module_globals.items():
                if inspect.isclass(obj) and (
                    ...
                    or any(method.startswith("test_") for method in dir(obj))
                ):
    ...
    def run_single_test_class(
        self, test_class, test_config: Optional[Dict[str, Any]] = None
    ) -> Dict:
        """
...
class NotebookTestEnvironment:
    ...
    def _setup_spark_session(self, use_real_spark: bool = False):
        ...
        if use_real_spark:
            ...
            self.spark_session = (
                SparkSession.builder.appName("NotebookTesting")
                .master("local[2]")
    ...
    def _setup_base_classes(self):
        ...
        if "WatermarkService" not in globals():
            ...
            class MockWatermarkManager:
                def __init__(
                    self, entity_provider=None, watermark_entity_finder=None, logger_provider=None
                ):
        ...
        if "SimpleReadPersistStrategy" not in globals():
            ...
            class MockSimpleReadPersistStrategy:
                def __init__(
                    self, watermark_service=None, entity_provider=None, logger_provider=None
                ):
        ...
        if "StageProcessingService" not in globals():
            ...
            class MockStageProcessingService:
                ...
                def execute(
                    self, stage: str, stage_description: str, stage_details: dict, layer: str
                ):
        ...
        if "ConfigService" not in globals():
            ...
            class MockConfigService:
                def __init__(self, config_data=None, logger_provider=None):
                    ...
                    self.get = MagicMock(
                        side_effect=lambda key, default=None: self.config_data.get(key, default)
                    )
                    self.set = MagicMock(
                        side_effect=lambda key, value: self.config_data.__setitem__(key, value)
                    )
        ...
        if "EntityNameMapper" not in globals():

            class MockEntityNameMapper:
                def __init__(self, config=None, logger_provider=None):
                    if logger_provider:
                        self.logger = logger_provider.get_logger("EntityNameMapper")
                    else:
                        self.logger = MagicMock()

                    self.config = config
                    self.default_database = "test_db"

                def get_table_name(self, entity):
                    """Generate table name from entity"""
                    ...
                    # Handle layer.entity_name format
                    if "." in entity_id:
                        layer, name = entity_id.split(".", 1)
                        return f"{self.default_database}.{layer}_{name}"
                    else:
                        return f"{self.default_database}.{entity_id}"

                def get_database_name(self, entity):
                    """Get database name for entity"""
                    return self.default_database

                def get_unqualified_table_name(self, entity):
                    """Get table name without database"""
                    full_name = self.get_table_name(entity)
                    if "." in full_name:
                        return full_name.split(".", 1)[1]
                    return full_name

            globals()["EntityNameMapper"] = MockEntityNameMapper

        if "EntityPathLocator" not in globals():

            class MockEntityPathLocator:
                def __init__(self, config=None, logger_provider=None):
                    if logger_provider:
                        self.logger = logger_provider.get_logger("EntityPathLocator")
                    else:
                        self.logger = MagicMock()

                    self.config = config
                    self.base_path = "abfss://test-container@teststorage.dfs.core.windows.net"
                    self.tables_root = f"{self.base_path}/tables"

                def get_table_path(self, entity):
                    """Generate table path from entity"""
                    if hasattr(entity, "entityid"):
                        entity_id = entity.entityid
                    elif hasattr(entity, "name"):
                        entity_id = entity.name
                    ...
                    if "." in entity_id:
                        layer, name = entity_id.split(".", 1)
                        return f"{self.tables_root}/{layer}/{name}"
                    else:
                        return f"{self.tables_root}/{entity_id}"

                def get_checkpoint_path(self, entity, stream_name=None):
                    """Generate checkpoint path for streaming"""
                    table_path = self.get_table_path(entity)
                    if stream_name:
                        return f"{table_path}/_checkpoints/{stream_name}"
                    return f"{table_path}/_checkpoints"

            globals()["EntityPathLocator"] = MockEntityPathLocator
...
class NotebookTestRunner:
    ...
    def assert_event_emitted(self, component: str, operation: str, count: int = 1):
        ...
        matching_calls = [
            call for call in calls if call[0][0] == component and call[0][1].startswith(operation)
        ]

        assert (
            len(matching_calls) >= count
    ...
    def assert_mdc_context_used(self, expected_keys: List[str]):
        ...
        for key in expected_keys:
            mdc_key = f"mdc.{key}"
            assert any(
                call[0][0] == mdc_key for call in mdc_calls
...
class SynapseNotebookTestCase:
    ...
    def real_spark_config(self):
        ...
        return {"use_real_spark": True, "test_data": {}}
            ...
            for test_class, method_name in all_tests:
                ...
                try:
                    ...
                    if current_spark_type != spark_type:
                        print(
                            f"🚨 WARNING: Spark type changed before {test_name}: {current_spark_type}"
                        )
