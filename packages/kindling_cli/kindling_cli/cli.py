...
def _azure_cloud_config() -> Dict[str, str]:
    cloud_name = os.getenv("AZURE_CLOUD") or "AzurePublicCloud"
    config = AZURE_CLOUD_CONFIGS[_normalize_azure_environment(cloud_name)]
    return {key: value for key, value in config.items() if key != "aliases"}


def _azure_authority_host() -> str:
    return os.getenv("AZURE_AUTHORITY_HOST", _azure_cloud_config()["authority"]).strip().rstrip("/")
...
def _resolve_account_url(storage_account: str) -> str:
    ...
    endpoint_suffix = _azure_cloud_config()["storage_suffix"]
...
def _get_blob_service_client(storage_account: str):
    ...
    credential = DefaultAzureCredential(
        authority=_azure_authority_host(), additionally_allowed_tenants=["*"]
    )
...
def _import_notebook_to_workspace(
    ...
    if platform == "databricks":
        ...
        cloud_config = _azure_cloud_config()
        credential = DefaultAzureCredential(
            authority=_azure_authority_host(), additionally_allowed_tenants=["*"]
        )
        synapse_scope = os.getenv("AZURE_SYNAPSE_TOKEN_SCOPE", cloud_config["synapse_scope"])
        token = credential.get_token(synapse_scope).token
        headers = {"Authorization": f"Bearer {token}", "Content-Type": "application/json"}

        synapse_suffix = os.getenv(
            "AZURE_SYNAPSE_DEV_ENDPOINT_SUFFIX", cloud_config["synapse_suffix"]
        )
