...
@click.option(
    ...
    show_default=True,
    type=click.Path(path_type=Path, file_okay=False),
    help="Path to the app root containing app.py.",
)
@click.option(
    "--module",
    "modules",
...
def app_add_executor(
    ...
    if not registration_modules:
        raise click.ClickException(
            "No package registration modules were found. "
            "Pass one or more --module values, for example "
            "`--module my_package.entities.records --module my_package.pipes.process`."
        )
