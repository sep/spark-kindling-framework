...
All notable changes to the kindling-ext-otel-azure extension will be documented in this file.
...
### Changed
- **BREAKING**: Collapsed three per-platform wheels (`kindling_ext_otel_azure_fabric`, `_synapse`, `_databricks`) into a single `kindling-ext-otel-azure` wheel. Pre-v0.4 wheels differed only in pin profile, not code — Fabric pinned the older OpenTelemetry line (1.6.x / 1.20.x) for compatibility with Fabric's azure-core 1.30.x runtime, Synapse/Databricks used the modern line (1.8.x / 1.21.x).
...
Initially attempted to express per-platform pin divergence via PEP 508 markers on optional extras. The extension install path in `packages/kindling/bootstrap.py` (for consumers listing `kindling-ext-otel-azure` under `kindling.extensions` in settings) runs `pip install <local_wheel>` without requesting extras, so extra-gated deps would silently skip and `import kindling_ext_otel_azure` would fail at runtime on every platform. Universal hard pins avoid this foot-gun.
...
kindling:
  extensions:
    - kindling-ext-otel-azure-fabric>=0.3.2   # or -synapse / -databricks
```
...
The Python import path (`from kindling_ext_otel_azure import ...`) is unchanged.
...
kindling:
  extensions:
    - kindling-ext-otel-azure>=0.3.0-alpha.1  # Updated version
```
