...
| Secret Scopes | `myproject-adls-scope` | None | **Create scope** |
...
- **Access Connector**: `/subscriptions/00000000-0000-0000-0000-000000000003/resourceGroups/sep-rg-dataint-dev/providers/Microsoft.Databricks/accessConnectors/sep-acad-db-dev`
- **Owner**: jtdossett@sep.com

### New
- **Name**: `sep_db_framework_poc_wcus` (auto-created default)
- **Access Connector**: `/subscriptions/00000000-0000-0000-0000-000000000003/resourcegroups/databricks-rg-sep-db-framework-poc-wcus-2cvrg7qkx3462/providers/Microsoft.Databricks/accessConnectors/unity-catalog-access-connector`

### Action
Create a new storage credential pointing to the **same** access connector (`sep-acad-db-dev`) that has access to `mystorageaccount` storage account, **OR** grant the new workspace's access connector (`unity-catalog-access-connector`) the `Storage Blob Data Contributor` role on `mystorageaccount`.

---

## 2. Unity Catalog — External Locations

### Original (5 locations, all on `mystorageaccount`)

| Name | URL | Credential |
|------|-----|------------|
| `artifacts` | `abfss://artifacts@mystorageaccount.dfs.core.windows.net/` | `datalake_azuremanagedidentity_*` |
| `bronze` | `abfss://bronze@mystorageaccount.dfs.core.windows.net/` | `datalake_azuremanagedidentity_*` |
| `gold` | `abfss://gold@mystorageaccount.dfs.core.windows.net/` | `datalake_azuremanagedidentity_*` |
| `landing` | `abfss://landing@mystorageaccount.dfs.core.windows.net/` | `datalake_azuremanagedidentity_*` |
| `silver` | `abfss://silver@mystorageaccount.dfs.core.windows.net/` | `datalake_azuremanagedidentity_*` |
...
- **Storage Root**: `abfss://artifacts@mystorageaccount.dfs.core.windows.net/catalog`
- **Owner**: jtdossett@sep.com
- **Schemas**:
  - `bronze` (owner: jtdossett@sep.com)
  - `default` (owner: jtdossett@sep.com)
  - `gold` (owner: jtdossett@sep.com)
  - `silver` (owner: jtdossett@sep.com, storage root: `abfss://silver@mystorageaccount.dfs.core.windows.net/`)
...
CREATE CATALOG medallion
  MANAGED LOCATION 'abfss://artifacts@mystorageaccount.dfs.core.windows.net/catalog';

CREATE SCHEMA medallion.bronze;
CREATE SCHEMA medallion.gold;
CREATE SCHEMA medallion.silver
  MANAGED LOCATION 'abfss://silver@mystorageaccount.dfs.core.windows.net/';
...
|------|------|-----------------|
| `config` | EXTERNAL | `abfss://artifacts@mystorageaccount.dfs.core.windows.net/config` |
| `data_apps` | EXTERNAL | `abfss://artifacts@mystorageaccount.dfs.core.windows.net/data-apps` |
| `logs` | EXTERNAL | `abfss://artifacts@mystorageaccount.dfs.core.windows.net/logs` |
| `packages` | EXTERNAL | `abfss://artifacts@mystorageaccount.dfs.core.windows.net/packages` |
| `scripts` | EXTERNAL | `abfss://artifacts@mystorageaccount.dfs.core.windows.net/scripts` |
| `temp` | MANAGED | (auto-assigned under catalog storage) |

### Action
```sql
CREATE EXTERNAL VOLUME medallion.default.config
  LOCATION 'abfss://artifacts@mystorageaccount.dfs.core.windows.net/config';

CREATE EXTERNAL VOLUME medallion.default.data_apps
  LOCATION 'abfss://artifacts@mystorageaccount.dfs.core.windows.net/data-apps';

CREATE EXTERNAL VOLUME medallion.default.logs
  LOCATION 'abfss://artifacts@mystorageaccount.dfs.core.windows.net/logs';

CREATE EXTERNAL VOLUME medallion.default.packages
  LOCATION 'abfss://artifacts@mystorageaccount.dfs.core.windows.net/packages';

CREATE EXTERNAL VOLUME medallion.default.scripts
  LOCATION 'abfss://artifacts@mystorageaccount.dfs.core.windows.net/scripts';
...
### Original
- **Scope**: `myproject-adls-scope` (backend: DATABRICKS)
...
databricks secrets create-scope myproject-adls-scope
...
2. Add Daniel Gomez (`admin@example.com`) to `admins` group
...
| Setting | Value |
|---------|-------|
| Source | Glob: `/Workspace/Users/admin@example.com/test_pipeline/transformations/**` |
| Catalog | `medallion` |
| Schema | `bronze` |
...
### Original
- `process_domain_job` (creator: admin@example.com) — likely a real/useful job
...
1. [ ] **Storage Credential** — Create managed identity credential pointing to `sep-acad-db-dev` access connector (or grant new connector access to `mystorageaccount`)
2. [ ] **External Locations** — Create 5 locations (artifacts, bronze, gold, landing, silver) on `mystorageaccount`
...
6. [ ] **Secret Scope** — Create `myproject-adls-scope`
7. [ ] **Service Principals** — Add sep-kindling-nonprod, data-app-poc, SEP Databricks Dev SP
8. [ ] **Users & Groups** — Add admin@example.com to admins, add all to users group
