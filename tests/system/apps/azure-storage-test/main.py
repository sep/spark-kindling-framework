...
class SynapseSystemTestApp:
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"✓ Spark session initialized (version {self.spark.version})")
            self.logger.info(
                f"✓ Platform: {self.platform_service.get_platform_name()}")
            ...
            self._add_error("initialization", error_msg,
    ...
    def run_azure_storage_tests(self) -> bool:
        ...
        for step_name, test_func in test_steps:
            ...
            try:
                ...
                if success:
                    self.logger.info(
                        f"✓ {step_name} completed successfully ({step_duration:.1f}s)")
                else:
                    self.logger.error(
                        f"❌ {step_name} failed ({step_duration:.1f}s)")
    ...
    def _test_basic_connectivity(self) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"✓ Read completed in {read_duration:.2f}s, rows: {row_count}")
    ...
    def _test_delta_operations(self) -> bool:
        ...
        try:
            ...
            initial_df = self.spark.createDataFrame([
                (1, "initial_record", 100.0),
                (2, "another_record", 200.0)
            ...
            self.logger.info(
                f"✓ Delta operations: initial={initial_count}, final={final_count}")

            self.test_results["metrics"].update({
                "delta_initial_rows": initial_count,
                "delta_final_rows": final_count
            })
    ...
    def _test_performance_write(self) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"Generating {row_count} rows in {partitions} partitions")

            # Create performance test data
            perf_df = self.spark.range(row_count).selectExpr(
                "id",
            ...
            estimated_size_mb = (row_count * 100) / \
                (1024 * 1024)  # Rough estimate
            throughput_mbps = estimated_size_mb / write_duration if write_duration > 0 else 0

            self.logger.info(
                f"✓ Performance write: {row_count} rows in {write_duration:.2f}s")
            self.logger.info(
                f"✓ Estimated throughput: {throughput_mbps:.2f} MB/s")
            ...
            min_throughput = float(
                os.getenv('MIN_WRITE_THROUGHPUT_MBPS', '15.0'))
    ...
    def _test_performance_read(self) -> bool:
        ...
        try:
            perf_path = self._get_test_path(
                "performance-write")  # Read from write test
            ...
            read_throughput_mbps = estimated_size_mb / \
                read_duration if read_duration > 0 else 0

            self.logger.info(
                f"✓ Performance read: {row_count} rows in {read_duration:.2f}s")
            self.logger.info(
                f"✓ Read throughput: {read_throughput_mbps:.2f} MB/s")
            ...
            # Check against minimum performance expectation
            min_read_throughput = float(
                os.getenv('MIN_READ_THROUGHPUT_MBPS', '60.0'))
    ...
    def _test_kindling_integration(self) -> bool:
        ...
        try:
            ...
            self.logger.info(
                "✓ Kindling framework components initialized successfully")
    ...
    def _cleanup_test_data(self) -> bool:
        ...
        try:
            cleanup_enabled = os.getenv(
                'CLEANUP_ON_SUCCESS', 'true').lower() == 'true'
            ...
            for path_suffix in test_paths:
                ...
                try:
                    ...
                    self.logger.warning(
                        f"Could not clean up {test_path}: {str(e)}")
    ...
    def _get_test_path(self, test_suffix: str) -> str:
        ...
        test_prefix = os.getenv(
            'TEST_PATH_PREFIX', 'system-tests/synapse/azure-storage')
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')

        return (f"abfss://{self.container}@{self.storage_account}."
        ...
        try:
            ...
            self.test_results["total_duration"] = (
                ...
                datetime.fromisoformat(
                    self.test_results["start_time"].replace('Z', '+00:00'))
            ).total_seconds()
    ...
    def _get_results_path(self) -> str:
        ...
        results_prefix = os.getenv(
            'RESULTS_PATH_PREFIX', 'system-test-results/azure-storage-test')
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
