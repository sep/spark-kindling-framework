...
class TestLocalServiceInitialization:
    ...
    def test_initialization_with_default_config(self, mock_logger):
        ...
        assert service.workspace_url == "http://localhost", "Workspace URL should be 'http://localhost'"
        assert service.local_workspace_path.name == "kindling_local_workspace", "Default workspace directory name should be 'kindling_local_workspace'"
        assert service.local_workspace_path.exists(
    ...
    def test_initialization_with_custom_workspace(self, temp_workspace, mock_logger):
        ...
        assert service.local_workspace_path == temp_workspace, f"Workspace path should be set to {temp_workspace}"
    ...
    def test_creates_workspace_directory(self, temp_workspace, mock_logger):
        ...
        assert service.local_workspace_path == new_workspace, f"Workspace path should be set to {new_workspace}"
...
class TestLocalServiceFileOperations:
    ...
    def test_write_and_read_text_file(self, temp_workspace, mock_logger):
        ...
        assert content == test_content, f"Read content should match written content: expected '{test_content}', got '{content}'"
    ...
    def test_write_and_read_binary_file(self, temp_workspace, mock_logger):
        ...
        assert content == test_content, f"Binary content should match: expected {test_content}, got {content}"
    ...
    def test_file_exists(self, temp_workspace, mock_logger):
        ...
        assert not service.exists(
            "nonexistent.txt"), "exists() should return False for nonexistent file"

        # Create file
        service.write("exists.txt", "content")
        assert service.exists(
            "exists.txt"), "exists() should return True after file is created"
    ...
    def test_delete_file(self, temp_workspace, mock_logger):
        ...
        assert service.exists(
            "to_delete.txt"), "File should exist after creation"

        # Delete file
        service.delete("to_delete.txt")
        assert not service.exists(
            "to_delete.txt"), "File should not exist after deletion"
    ...
    def test_copy_file(self, temp_workspace, mock_logger):
        ...
        assert service.exists(
            "source.txt"), "Source file should still exist after copy"
        assert service.exists(
            "destination.txt"), "Destination file should exist after copy"

        # Content should match
        assert service.read(
            "destination.txt") == source_content, "Copied file content should match source"
    ...
    def test_move_file(self, temp_workspace, mock_logger):
        ...
        assert not service.exists(
            "source.txt"), "Source file should not exist after move"
        assert service.exists(
            "moved.txt"), "Destination file should exist after move"
        assert service.read(
            "moved.txt") == source_content, "Moved file content should match original"
    ...
    def test_write_with_subdirectories(self, temp_workspace, mock_logger):
        ...
        assert service.exists(
            "subdir1/subdir2/file.txt"), "File should exist in nested subdirectories"
        assert service.read(
            "subdir1/subdir2/file.txt") == "content", "Content should be readable from nested path"
...
class TestLocalServiceNotebooks:
    ...
    def test_get_notebook_with_content(self, temp_workspace, mock_logger, sample_notebook_files):
        ...
        assert notebook is not None, "get_notebook should return a notebook object"
        assert hasattr(notebook.properties,
                       'cells'), "Notebook properties should have 'cells' attribute when include_content=True"
        assert len(
            notebook.properties.cells) > 0, "Notebook should have at least one cell"
...
class TestLocalServicePaths:
    ...
    def test_relative_path_handling(self, temp_workspace, mock_logger):
        ...
        assert service.exists(
            "relative.txt"), "File should exist using relative path"
        assert service.read(
            "relative.txt") == "content", "Content should be readable using relative path"
    ...
    def test_absolute_path_handling(self, temp_workspace, mock_logger):
        ...
        assert service.exists(
            str(abs_path)), "File should exist using absolute path"
        assert service.read(
            str(abs_path)) == "content", "Content should be readable using absolute path"
...
class TestLocalServiceInfo:
    ...
    def test_get_workspace_info(self, temp_workspace, mock_logger):
        ...
        assert info['workspace_url'] == "http://localhost", "workspace_url should be 'http://localhost'"
    ...
    def test_get_token(self, temp_workspace, mock_logger):
        ...
        assert token == "local-dummy-token", "get_token() should return 'local-dummy-token' for local platform"
...
class TestLocalServiceEdgeCases:
    ...
    def test_write_overwrite_protection(self, temp_workspace, mock_logger):
        ...
        assert service.read(
            "protected.txt") == "original", "File content should remain unchanged when overwrite=False raises error"
    ...
    def test_write_with_overwrite(self, temp_workspace, mock_logger):
        ...
        assert service.read(
            "replaceable.txt") == "new content", "File content should be updated when overwrite=True"
