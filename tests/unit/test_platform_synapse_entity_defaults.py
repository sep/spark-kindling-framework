...
from kindling.platform_synapse import (
    SynapseEntityNameMapper,
    SynapseEntityPathLocator,
...
def test_bind_default_entity_services_binds_synapse_defaults():
    logger = MagicMock()
    injector, binder = _build_injector(False, False)

    with patch("kindling.platform_synapse.GlobalInjector.get_injector", return_value=injector):
    ...
    assert first.kwargs["to"] is SynapseEntityNameMapper

    assert second.args[0] is EntityPathLocator
    assert second.kwargs["to"] is SynapseEntityPathLocator
...
class TestSynapseEntityNameMapper:
    def test_uses_provider_table_name_tag_when_present(self):
        ...
        mapper = SynapseEntityNameMapper(config, logger_provider)
        result = mapper.get_table_name(entity)
        assert result == "sales.orders"

    def test_uses_configured_schema(self):
        values = {"kindling.synapse.schema": "silver"}
        config = MagicMock()
        ...
        mapper = SynapseEntityNameMapper(config, logger_provider)
        result = mapper.get_table_name(entity)
        assert result == "silver.stream_orders"
...
class TestSynapseEntityPathLocator:
