...
from kindling.platform_synapse import _bind_default_entity_services
...
def test_bind_default_entity_services_sets_delta_access_default_when_missing():
    ...
    with patch("kindling.platform_synapse.GlobalInjector.get", return_value=cs):
...
class TestConfigDrivenEntityNameMapperSynapseFallbacks:
    def test_uses_provider_table_name_tag_when_present(self):
        ...
        entity = MagicMock()
        entity.tags = {"provider.table_name": "silver.orders"}
        entity.entityid = "stream.orders"

        mapper = ConfigDrivenEntityNameMapper(config, logger_provider)
        assert mapper.get_table_name(entity) == "silver.orders"

    def test_uses_synapse_schema_when_storage_namespace_not_set(self):
        values = {
            "kindling.storage.table_catalog": None,
            "kindling.storage.table_schema": None,
            "kindling.synapse.schema": "silver",
        ...
        mapper = ConfigDrivenEntityNameMapper(config, logger_provider)
        assert mapper.get_table_name(entity) == "silver.stream_orders"


class TestConfigDrivenEntityPathLocatorSynapseFallbacks:
    ...
    def test_uses_synapse_table_root_when_storage_root_not_set(self):
        values = {
            "kindling.storage.table_root": None,
            "kindling.synapse.table_root": "Tables/base",
        }
        config = MagicMock(spec=ConfigService)
        config.get.side_effect = lambda key, default=None: values.get(key, default)
