...
class TestEntityMetadata:
    ...
    def test_entity_metadata_has_required_fields(self):
        ...
        ), f"EntityMetadata fields mismatch. Expected: {expected_fields}, Got: {metadata_fields}"
    ...
    def test_entity_metadata_field_types(self):
        ...
        assert (
            "partition_columns" in field_annotations
        ), "EntityMetadata should have 'partition_columns' field"
        assert (
            "merge_columns" in field_annotations
    ...
    def test_entity_metadata_creation(self):
        ...
        assert metadata.partition_columns == [
            "date",
            "region",
        ], "EntityMetadata.partition_columns should be ['date', 'region']"
        assert metadata.merge_columns == ["id"], "EntityMetadata.merge_columns should be ['id']"
        assert metadata.tags == {
            "source": "test",
            "owner": "data_team",
        }, "EntityMetadata.tags should match expected dict"
    ...
    def test_entity_metadata_complex_schema(self):
        ...
        assert (
            metadata.schema == complex_schema
        ), "schema should equal the complex_schema StructType"
        assert isinstance(metadata.schema, StructType), "schema should be instance of StructType"
...
class TestDataEntityManager:
    ...
    def test_initialization(self):
        ...
        assert hasattr(manager, "registry"), "DataEntityManager should have 'registry' attribute"
        assert isinstance(manager.registry, dict), "registry should be a dict"
        assert len(manager.registry) == 0, "registry should be empty after initialization"
    ...
    def test_register_entity(self):
        ...
        ), "Registered entity should be EntityMetadata instance"
        ...
        assert entity.partition_columns == ["date"], "Entity partition_columns should be ['date']"
        assert entity.merge_columns == ["id"], "Entity merge_columns should be ['id']"
        assert entity.tags == {"env": "test"}, "Entity tags should be {'env': 'test'}"
        assert entity.schema == {"id": "string"}, "Entity schema should be {'id': 'string'}"
    ...
    def test_register_entity_overwrites_existing(self):
        ...
        assert len(manager.registry) == 1, "Registry should only contain 1 entity after overwrite"
        ...
        assert entity.partition_columns == [
            "v2"
        ], "Entity partition_columns should be updated to ['v2']"
        assert entity.tags == {"version": "2"}, "Entity tags should be updated to {'version': '2'}"
    ...
    def test_get_entity_ids_empty(self):
        ...
        assert len(list(entity_ids)) == 0, "Empty registry should return no entity IDs"
    ...
    def test_get_entity_ids(self):
        ...
        manager.register_entity(
            "entity3", name="E3", partition_columns=[], merge_columns=[], tags={}, schema={}
        )
    ...
    def test_get_entity_definition_existing(self):
        ...
        assert isinstance(
            entity, EntityMetadata
        ), "Returned entity should be EntityMetadata instance"
    ...
    def test_implements_data_entity_registry_interface(self):
        ...
        assert issubclass(
            DataEntityManager, DataEntityRegistry
        ), "DataEntityManager should be subclass of DataEntityRegistry"

        # Check all abstract methods are implemented
        manager = DataEntityManager()
        assert callable(manager.register_entity), "register_entity should be callable"
        assert callable(manager.get_entity_ids), "get_entity_ids should be callable"
        assert callable(manager.get_entity_definition), "get_entity_definition should be callable"
...
class TestDataEntitiesDecorator:
    ...
    def test_decorator_validates_required_fields(self):
        ...
        assert (
            "Missing required fields" in error_message
    ...
    def test_decorator_validates_partial_missing_fields(self):
        ...
        assert (
            "name" not in error_message or "Missing" not in error_message
        ), "Error should not mention provided 'name' field"
    ...
    def test_decorator_removes_entityid_from_params(self):
        ...
        assert (
            call_args[0][0] == "test_entity"
        ), "First positional arg should be entityid value 'test_entity'"
        assert (
            "entityid" not in call_args[1]
        ), "entityid should not be in kwargs passed to register_entity"
    ...
    def test_decorator_lazy_initializes_registry(self):
        ...
        with patch.object(GlobalInjector, "get", return_value=mock_registry) as mock_get:
            ...
            mock_get.assert_called_once(), "Should call GlobalInjector.get exactly once to initialize registry"

            # Registry should be set
            assert (
                DataEntities.deregistry == mock_registry
            ), "deregistry should be set to the registry from GlobalInjector"
    ...
    def test_decorator_reuses_existing_registry(self):
        ...
        with patch.object(GlobalInjector, "get") as mock_get:
            ...
            mock_get.assert_not_called(), "Should not call GlobalInjector.get when registry already exists"

            # Should use the existing registry
            mock_registry.register_entity.assert_called_once(), "Should call register_entity on existing registry"
...
class TestAbstractBaseClasses:
    ...
    def test_entity_path_locator_interface(self):
        ...
        assert issubclass(EntityPathLocator, ABC), "EntityPathLocator should be subclass of ABC"
    ...
    def test_entity_name_mapper_interface(self):
        ...
        assert issubclass(EntityNameMapper, ABC), "EntityNameMapper should be subclass of ABC"
    ...
    def test_entity_provider_interface(self):
        ...
        assert issubclass(EntityProvider, ABC), "EntityProvider should be subclass of ABC"
    ...
    def test_entity_provider_has_all_required_methods(self):
        ...
        for method_name in required_methods:
            assert hasattr(
                EntityProvider, method_name
            ), f"EntityProvider should have abstract method '{method_name}'"

    def test_data_entity_registry_interface(self):
        """Test DataEntityRegistry has required abstract methods"""
        from abc import ABC

        assert issubclass(DataEntityRegistry, ABC), "DataEntityRegistry should be subclass of ABC"
        ...
        ), "DataEntityRegistry should have 'register_entity' method"
        assert hasattr(
            DataEntityRegistry, "get_entity_ids"
        ), "DataEntityRegistry should have 'get_entity_ids' method"
        assert hasattr(
            DataEntityRegistry, "get_entity_definition"
        ), "DataEntityRegistry should have 'get_entity_definition' method"
...
class TestEntityMetadataIntegration:
    ...
    def test_full_registration_workflow(self):
        ...
        assert len(list(manager.get_entity_ids())) == 3, "Should have 3 registered entities"
        ...
        for entity_data in entities_to_register:
            ...
            assert (
                entity.entityid == entity_data["entityid"]
            ), f"Entity entityid should match '{entity_data['entityid']}'"
            assert (
                entity.name == entity_data["name"]
            ), f"Entity name should match '{entity_data['name']}'"
            assert (
                entity.partition_columns == entity_data["partition_columns"]
            ), f"Entity partition_columns should match for '{entity_data['entityid']}'"
            assert (
                entity.merge_columns == entity_data["merge_columns"]
            ), f"Entity merge_columns should match for '{entity_data['entityid']}'"
            assert (
                entity.tags == entity_data["tags"]
            ), f"Entity tags should match for '{entity_data['entityid']}'"
            assert (
                entity.schema == entity_data["schema"]
            ), f"Entity schema should match for '{entity_data['entityid']}'"
...
class TestDataEntitiesEdgeCases:
    ...
    def test_entity_metadata_is_dataclass(self):
        ...
        assert is_dataclass(EntityMetadata), "EntityMetadata should be a dataclass"
