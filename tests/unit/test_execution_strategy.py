...
from kindling.pipe_graph import PipeGraph, PipeNode, PipeEdge, PipeGraphBuilder
...
class TestExecutionPlanGenerator:
    ...
    def test_complex_workflow(self, generator, pipe_registry):
        ...
        self.register_pipe(pipe_registry, "silver_customers", ["bronze.customers"], "silver.customers")
        self.register_pipe(
            pipe_registry,
            "gold_analytics",
    ...
    def test_generate_config_based_plan(self, generator, pipe_registry):
        """Test generating config-based plan with priorities."""
        # Register pipes with priority tags
        pipe_registry.register_pipe(
            "high_priority",
            name="high_priority",
            execute=lambda: None,
            tags={"execution_priority": "1"},
            input_entity_ids=[],
        ...
        pipe_registry.register_pipe(
            ...
            name="low_priority",
            execute=lambda: None,
            tags={"execution_priority": "10"},
            input_entity_ids=[],
            output_entity_id="entity.b",
            output_type="delta"
        )
        
        plan = generator.generate_config_based_plan(["high_priority", "low_priority"])
        
        assert plan.strategy == "config_based"
        assert plan.total_generations() == 2
        # High priority (1) executes before low priority (10)
        assert plan.generations[0].pipe_ids == ["high_priority"]
        assert plan.generations[1].pipe_ids == ["low_priority"]
