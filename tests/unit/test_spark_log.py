...
class TestSparkLoggerInitialization:
    ...
    def test_initialization_with_all_parameters(self, mock_spark, mock_logger, basic_config):
        ...
        assert len(
            logger._level_hierarchy) == 4, "Log level hierarchy should contain all four levels"
    ...
    def test_initialization_without_spark_session(self, mock_logger, basic_config):
        ...
        assert logger.name == "TestLogger", "Logger should initialize when Spark session is auto-created"
        assert logger.logger == mock_logger, "Base logger should still be set when Spark session is auto-created"
    ...
    def test_log_level_hierarchy_values(self, mock_spark, mock_logger, basic_config):
        ...
        assert logger._level_hierarchy[
            'error'] == 0, "Error should have highest priority (0)"
        assert logger._level_hierarchy[
            'warn'] == 1, "Warn should have second priority (1)"
        assert logger._level_hierarchy[
            'info'] == 2, "Info should have third priority (2)"
        assert logger._level_hierarchy[
            'debug'] == 3, "Debug should have lowest priority (3)"
        assert logger._level_hierarchy['error'] < logger._level_hierarchy[
            'debug'], "Error priority should be higher than debug"
...
class TestLogLevelFiltering:
    ...
    def test_is_level_enabled_with_info_level(self, mock_spark, mock_logger, basic_config):
        ...
        assert logger._is_level_enabled(
            'error') is True, "Error should be enabled when effective level is INFO"
        assert logger._is_level_enabled(
            'warn') is True, "Warn should be enabled when effective level is INFO"
        assert logger._is_level_enabled(
            'info') is True, "Info should be enabled when effective level is INFO"
        assert logger._is_level_enabled(
            'debug') is False, "Debug should be disabled when effective level is INFO"
    ...
    def test_is_level_enabled_with_warn_level(self, mock_spark, mock_logger, basic_config):
        ...
        assert logger._is_level_enabled(
            'error') is True, "Error should be enabled when effective level is WARN"
        assert logger._is_level_enabled(
            'warn') is True, "Warn should be enabled when effective level is WARN"
        assert logger._is_level_enabled(
            'info') is False, "Info should be disabled when effective level is WARN"
        assert logger._is_level_enabled(
            'debug') is False, "Debug should be disabled when effective level is WARN"
    ...
    def test_is_level_enabled_with_error_level(self, mock_spark, mock_logger, basic_config):
        ...
        assert logger._is_level_enabled(
            'error') is True, "Error should be enabled when effective level is ERROR"
        assert logger._is_level_enabled(
            'warn') is False, "Warn should be disabled when effective level is ERROR"
        assert logger._is_level_enabled(
            'info') is False, "Info should be disabled when effective level is ERROR"
        assert logger._is_level_enabled(
            'debug') is False, "Debug should be disabled when effective level is ERROR"
    ...
    def test_is_level_enabled_with_debug_level(self, mock_spark, mock_logger, basic_config):
        ...
        assert logger._is_level_enabled(
            'error') is True, "Error should be enabled when effective level is DEBUG"
        assert logger._is_level_enabled(
            'warn') is True, "Warn should be enabled when effective level is DEBUG"
        assert logger._is_level_enabled(
            'info') is True, "Info should be enabled when effective level is DEBUG"
        assert logger._is_level_enabled(
            'debug') is True, "Debug should be enabled when effective level is DEBUG"
    ...
    def test_is_level_enabled_with_unknown_level(self, mock_spark, mock_logger, basic_config):
        ...
        assert logger._is_level_enabled(
            'debug') is True, "Unknown level should default to allowing debug"
        assert logger._is_level_enabled(
            'info') is True, "Unknown level should default to allowing info"
        assert logger._is_level_enabled(
            'warn') is True, "Unknown level should default to allowing warn"
        assert logger._is_level_enabled(
            'error') is True, "Unknown level should default to allowing error"
...
class TestLoggingMethods:
    ...
    def test_debug_method_calls_log(self, mock_spark, mock_logger, basic_config):
        ...
        mock_log.assert_called_once_with(
            'debug', 'debug message'), "_log should be called with 'debug' level and message"
    ...
    def test_info_method_calls_log(self, mock_spark, mock_logger, basic_config):
        ...
        mock_log.assert_called_once_with(
            'info', 'info message'), "_log should be called with 'info' level and message"
    ...
    def test_warn_method_calls_log(self, mock_spark, mock_logger, basic_config):
        ...
        mock_log.assert_called_once_with(
            'warn', 'warn message'), "_log should be called with 'warn' level and message"
    ...
    def test_warning_method_calls_log(self, mock_spark, mock_logger, basic_config):
        ...
        mock_log.assert_called_once_with(
            'warn', 'warning message'), "warning() should be alias calling _log with 'warn' level"
    ...
    def test_error_method_calls_log(self, mock_spark, mock_logger, basic_config):
        ...
        mock_log.assert_called_once_with(
            'error', 'error message'), "_log should be called with 'error' level and message"
    ...
    def test_all_methods_called_sequentially(self, mock_spark, mock_logger, basic_config):
        ...
        assert calls[0] == call(
            'debug', 'debug msg'), "First call should be debug"
        assert calls[1] == call(
            'info', 'info msg'), "Second call should be info"
        assert calls[2] == call(
            'warn', 'warn msg'), "Third call should be warn"
        assert calls[3] == call(
            'warn', 'warning msg'), "Fourth call should be warn (warning alias)"
        assert calls[4] == call(
            'error', 'error msg'), "Fifth call should be error"
...
class TestCentralLogMethod:
    ...
    def test_log_skips_when_level_disabled(self, mock_spark, mock_logger, basic_config):
        ...
        ), "Message formatting should be skipped when level is disabled for performance"
        mock_logger.debug.assert_not_called(
        ), "Base logger should not be called when level is disabled"
    ...
    def test_log_formats_and_logs_when_enabled(self, mock_spark, mock_logger, basic_config):
        ...
        mock_logger.info.assert_called_once_with(
            "formatted message"), "Base logger.info should be called with formatted message"

    def test_log_calls_correct_logger_method_for_each_level(self, mock_spark, mock_logger, basic_config):
        ...
        mock_logger.debug.assert_called_once_with(
            "msg"), "logger.debug should be called for debug level"
        mock_logger.info.assert_called_once_with(
            "msg"), "logger.info should be called for info level"
        mock_logger.warn.assert_called_once_with(
            "msg"), "logger.warn should be called for warn level"
        mock_logger.error.assert_called_once_with(
            "msg"), "logger.error should be called for error level"
    ...
    def test_log_prints_when_print_logging_enabled(self, mock_spark, mock_logger, basic_config):
        ...
        mock_print.assert_called_once_with(
            "formatted msg"), "print should be called with formatted message when print_logging is True"

    def test_log_does_not_print_when_print_logging_disabled(self, mock_spark, mock_logger, basic_config):
    ...
    def test_log_handles_missing_config(self, mock_spark, mock_logger):
        ...
        mock_logger.info.assert_called_once_with(
            "msg"), "Logging should work with empty dict config"
        mock_print.assert_not_called(
        ), "Print should not be called when config defaults to empty dict"


class TestPatternFormatting:
    """Test message pattern formatting."""

    def test_format_msg_with_simple_message_placeholder(self, mock_spark, mock_logger, basic_config):
    ...
    def test_format_msg_with_logger_name_full(self, mock_spark, mock_logger, basic_config):
        ...
        logger = SparkLogger("com.example.package.MyClass",
                             mock_logger, mock_spark, basic_config)
        logger.pattern = "[%c] %m"

        result = logger._format_msg("test", "info")

        assert "com.example.package.MyClass" in result, "Full logger name should be included with %c pattern"
    ...
    def test_format_msg_with_logger_name_truncated(self, mock_spark, mock_logger, basic_config):
        ...
        assert result_1.startswith(
            "MyClass"), "Logger name should be truncated to last 1 segment"
        assert "package.MyClass" in result_2, "Logger name should be truncated to last 2 segments"
        assert "example.package.MyClass" in result_3, "Logger name should be truncated to last 3 segments"
    ...
    def test_format_msg_with_date_placeholder(self, mock_spark, mock_logger, basic_config):
        ...
        assert re.search(
            r'\d{4}-\d{2}', result), "Date should contain year and month in YYYY-MM format"
    ...
    def test_format_msg_with_mdc_placeholders(self, mock_spark, mock_logger, basic_config):
        ...
        mock_spark.sparkContext.getLocalProperty = Mock(side_effect=lambda key: {
        ...
        logger = SparkLogger("TestLogger", mock_logger,
                             mock_spark, basic_config)
        logger.pattern = "trace=%x{trace_id} span=%x{span_id} comp=%x{component} op=%x{operation} %m"
    ...
    def test_format_msg_with_mdc_without_spark_session(self, mock_logger, basic_config):
        ...
        with patch('kindling.spark_log.get_or_create_spark_session', return_value=mock_spark_no_context):
            logger = SparkLogger("TestLogger", mock_logger, None, basic_config)
        logger.pattern = "trace=%x{trace_id} %m"

        result = logger._format_msg("test", "info")

        assert "trace=n/a" in result, "MDC placeholder should default to 'n/a' when Spark session lacks sparkContext"
    ...
    def test_format_msg_with_complex_pattern(self, mock_spark, mock_logger, basic_config):
        ...
        assert re.search(
            r'\d{4}-\d{2}', result), "Date should be formatted in pattern"
    ...
    def test_format_msg_with_static_pattern(self, mock_spark, mock_logger, basic_config):
        ...
        assert result == "static log text", "Pattern without placeholders should return literal text"
...
class TestPatternCustomization:
    ...
    def test_with_pattern_sets_custom_pattern(self, mock_spark, mock_logger, basic_config):
        ...
        assert logger.pattern == expected_pattern, "Custom pattern should be appended with MDC fields"
    ...
    def test_with_pattern_allows_method_chaining(self, mock_spark, mock_logger, basic_config):
        ...
        mock_log.assert_called_once_with(
            'info', 'test'), "Chained logging method should work"
...
class TestShouldPrint:
    ...
    def test_should_print_returns_false_when_disabled(self, mock_spark, mock_logger, basic_config):
        ...
        assert result is False, "should_print should return False when config.print_logging is False"
...
class TestEdgeCases:
    ...
    def test_logger_name_with_single_segment(self, mock_spark, mock_logger, basic_config):
        ...
        logger = SparkLogger("SimpleLogger", mock_logger,
                             mock_spark, basic_config)
        logger.pattern = "%c{2} %m"

        result = logger._format_msg("test", "info")

        assert "SimpleLogger" in result, "Single-segment name should be preserved even with truncation"
    ...
    def test_logger_name_truncation_exceeds_segments(self, mock_spark, mock_logger, basic_config):
        ...
        logger = SparkLogger("com.example", mock_logger,
                             mock_spark, basic_config)
    ...
    def test_pattern_with_percentage_not_followed_by_placeholder(self, mock_spark, mock_logger, basic_config):
        ...
        assert "100% complete:" in result, "Percentage signs not part of placeholders should be preserved"
