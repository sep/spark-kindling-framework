...
Tests that the spark-kindling-ext-otel-azure extension correctly overrides
...
def log_extension_version(logger):
    """Log the version of the spark-kindling-ext-otel-azure extension that was loaded"""
    try:
        import kindling_ext_otel_azure

        version = kindling_ext_otel_azure.__version__
        # Print to stdout (captured by Fabric/Databricks)
        print(f"EXTENSION_VERSION: spark-kindling-ext-otel-azure {version}")
        # Also log via framework
        logger.info(f"Extension loaded: spark-kindling-ext-otel-azure version {version}")
        return version
    except ImportError:
        print("⚠️  spark-kindling-ext-otel-azure not found - extension not loaded")
        logger.warning("spark-kindling-ext-otel-azure not found - extension not loaded")
        return None
    except AttributeError:
        print("⚠️  spark-kindling-ext-otel-azure version not available")
        logger.warning("spark-kindling-ext-otel-azure version not available")
        return None
...
if extension_version:
    ...
    print(f"✅ Extension package found: spark-kindling-ext-otel-azure {extension_version}")
else:
